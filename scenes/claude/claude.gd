...
func ShowWelcomeMessageIfNeeded():
	# Only show welcome message if no history and no API key configured
	if _claudeManager.GetFullHistory().size() == 0:
		if !_claudeManager.HasApiKey():
			%OutputRichTextLabel.text = "[color=#6bcfff][b]Welcome to Claude API Chat![/b][/color]\n\n"
			%OutputRichTextLabel.text += "To get started, you'll need to configure your Claude API key:\n\n"
			%OutputRichTextLabel.text += "1. Click the [color=orange]settings icon[/color] (gear) in the bottom right\n"
			%OutputRichTextLabel.text += "2. Enter your Claude API key\n"
			%OutputRichTextLabel.text += "3. Check 'Save Key Locally' to remember it\n"
			%OutputRichTextLabel.text += "4. Click 'Back' to save\n\n"
			%OutputRichTextLabel.text += "You can get an API key from [color=#6bcfff]console.anthropic.com[/color]\n\n"
...
func SendToClaude(textInput: String):
	if !_claudeManager.HasApiKey():
		%OutputRichTextLabel.text += "[color=red]Error: No API key configured. Click the settings icon (bottom right) to add your Claude API key.[/color]\n\n"
		StopAnimatingClaudeBusy()
		return
...
func _on_http_request_completed(_result: int, responseCode: int, _headers: PackedStringArray, body: PackedByteArray):
	if responseCode == 200:
		...
		var errorBody = body.get_string_from_utf8()
		%OutputRichTextLabel.text += "[color=red]API Error: " + str(responseCode) + "[/color]\n"
		if str(responseCode) == "401":
			%OutputRichTextLabel.text += "Invalid API key. Check your Claude API Key in settings (bottom right).\n"
		elif str(responseCode) == "404":
			%OutputRichTextLabel.text += "Endpoint not found. This may indicate an invalid model or API issue.\n"
		if errorBody != "":
			%OutputRichTextLabel.text += "Details: " + errorBody + "\n"
		%OutputRichTextLabel.text += "\n"
		StopAnimatingClaudeBusy()
