# Code Quality Manager - Settings Card UI Builder
# Adapted from GDScript Linter for standalone app usage
extends RefCounted
## Creates settings panel cards with consistent styling

const CollapsibleCard = preload("res://scenes/code-quality-manager/ui/collapsible-card.gd")
const ClaudeCodeCardBuilder = preload("res://scenes/code-quality-manager/ui/claude-code-card-builder.gd")
const HelpCardBuilder = preload("res://scenes/code-quality-manager/ui/help-card-builder.gd")
...
var _claude_card_builder
var _help_card_builder


func _init(reset_icon: Texture2D) -> void:
	_reset_icon = reset_icon
	_claude_card_builder = ClaudeCodeCardBuilder.new(reset_icon)
	_help_card_builder = HelpCardBuilder.new()
...
func build_settings_panel(settings_panel: PanelContainer, controls: Dictionary) -> void:
	...
	# Style scrollbar to match Project Manager
	var v_scrollbar := scroll.get_v_scroll_bar()
	v_scrollbar.custom_minimum_size = Vector2(20, 0)
...
func create_display_options_card(controls: Dictionary) -> VBoxContainer:
	var card = CollapsibleCard.new("Display Options", "code_quality/ui/display_options_collapsed")
...
func create_scan_options_card(controls: Dictionary) -> VBoxContainer:
	var card = CollapsibleCard.new("Scan Options", "code_quality/ui/scan_options_collapsed")
	var vbox := card.get_content_container()
	...
	controls.remember_filters_check = _create_checkbox("Remember Filters", hbox,
		"Persist Severity, Type, and Filter selections across sessions")

	return card


# Create Code Checks collapsible card with toggles for all analysis checks
func create_code_checks_card(controls: Dictionary) -> VBoxContainer:
	var card = CollapsibleCard.new("Code Checks", "code_quality/ui/code_checks_collapsed")
	var vbox := card.get_content_container()
...
func create_limits_card(controls: Dictionary) -> VBoxContainer:
	var card = CollapsibleCard.new("Analysis Limits", "code_quality/ui/limits_collapsed")
	var vbox := card.get_content_container()
...
func create_header_bar() -> HBoxContainer:
	...
	# Title: "Code Quality" in accent color
	var title := Label.new()
...
func _create_help_card() -> VBoxContainer:
	var card = CollapsibleCard.new("Help", "code_quality/ui/help_collapsed")
