...
func showDialog(butlerInfo: Dictionary):
	if butlerInfo.installed:
		_statusLabel.text = "Butler Status: Installed (%s)" % butlerInfo.version
		_statusLabel.add_theme_color_override("font_color", Color(0.4, 0.9, 0.4))
	else:
		_statusLabel.text = "Butler Status: Not Installed"
		_statusLabel.add_theme_color_override("font_color", Color(0.9, 0.4, 0.4))
